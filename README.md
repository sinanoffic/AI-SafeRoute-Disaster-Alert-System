...
2. **Frontend Map Simulation**: The frontend performs the main map simulation locally, computing risk scores using a deterministic arithmetic heuristic.
...
6. **Backend API**: The backend provides separate prototype API functionality. Notably, its `/api/risk` endpoint implements its own simplified linear formula, and the frontend main map simulation does not depend on that backend risk formula.
...
$ S_{raw}(t) = \Big[ R(t)^\alpha \cdot e^{\beta M} \Big] W_r + \Big[ P^\gamma \cdot D^{-\delta} \Big] W_p - \Big[ E^\epsilon \cdot L \Big] W_e + \lambda (\Delta R) $
...
* $E$: Elevation Factor
* $L$: Permeability
* $\Delta R$: Rainfall difference (event-based delta between slider updates, not a true time derivative $\Delta R/\Delta t$)
