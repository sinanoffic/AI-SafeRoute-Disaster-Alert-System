...
The overall raw score {raw}(t)$ is calculated using a nonlinear combination of terms:
* **Rainfall-Moisture ((t)^\alpha \cdot e^{\beta M}$):** Evaluates rainfall accumulation scaled exponentially by soil moisture.
* **Water & Drainage (^\gamma \cdot D^{-\delta}$):** Measures proximity to water coupled with drainage deficit.
* **Topography (^\epsilon \cdot L$):** Factors in elevation against surface permeability.
* **Rainfall Surge ($\lambda \cdot \Delta R$):** A response to acute increases in rainfall rate, based on event-to-event rainfall difference from slider updates (not a true time derivative $\Delta R/\Delta t$).
...
The system projects future risk by combining the current risk score with the current rainfall magnitude and water-proximity contribution across the selected time horizon. This is a simple arithmetic projection and does not employ advanced hydrodynamic forecasting.

## 6. Adaptive Weight Heuristic
The system features a simple deterministic heuristic that alters the rainfall weight ($) and proximity weight ($) slightly based on recent trends in maximum risk scores over the last few simulation snapshots. The elevation weight ($) is not adapted. This is a hardcoded arithmetic adjustment, and should not be confused with machine learning or actual AI model training.